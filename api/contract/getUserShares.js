...
export default async function handler(req, res) {
  ...
  const { drawId, user, threshold } = req.query;

  if (!drawId || !user || !threshold) {
    return res.status(400).json({ error: 'drawId, user, and threshold are required' });
  }
  ...
  try {
    ...
    const result = await client.readContract({
      ...
      args: [BigInt(drawId), user, BigInt(threshold)],
    });
